...
const PrincipalViewStudents = () => {
    ...
    const fetchStudentDetails = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get('/principal/viewstudents', {
                headers: {
    ...
    const fetchTeacherDetails = async (classId) => {
        try {
            console.log("Hello")
            const response = await axiosInstance.get(`/principal/teacher/${classId}`, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorage.getItem("token")}`
                },
